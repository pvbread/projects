...
def generate_stacks():
	left, right, middle = Stack("L"), Stack("R"), Stack("M")
	stacks = {'L':left,'M':middle,'R':right}
	return stacks

def show_stacks(stacks):
	print('\n...Current Stacks...')
	for stack in stacks.keys():
		stacks.get(stack).print_items()
...
def valid_choice(curr_choice, stacks, prev_choice=False):
	...
	#moving origin
	if not prev_choice:
		if stacks[curr_choice].is_empty():
	...
	#moving destination
		...
		if stacks[curr_choice].is_empty():
			return True
		elif stacks[curr_choice].peek() < stacks[prev_choice.upper()].peek():
...
def victory_condition(stacks):
	if stacks['L'].is_empty() and stacks['M'].is_empty():
...
def gameplay(stacks):
	...
	stacks[second_choice.upper()].push(stacks[first_choice.upper()].pop())
